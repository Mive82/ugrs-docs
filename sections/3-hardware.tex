...
\subsection{Hardver za komunikaciju}
...
\subsection{Prenošenje programske podrške na  ESP32}

Prenošenje programske podrške na ESP32 obavlja se putem UART sučelja. Kako bi se programska podrška mogla prenjeti, ESP se prvo mora staviti u režim preuzimanja koda. To se može postići tako da se nožice \texttt{GPIO00} i \texttt{GPIO02} postave na nisku naopnsku razinu prilikom spajanja ploče na izvor napajanja. Na našoj ploči to se može ostvariti na dva načina: ručno pritiskom na gumb \texttt{BOOT} dok se ploča ponovno pokrene pritiskom na gumb \texttt{EN}, ili spajanjem signala \texttt{DTR} i \texttt{RTS} UART adaptera na odgovarajuće nožice na JST programskoj spojnici. Oni su povezani u unutrašnjosti s ulazima \texttt{EN} i \texttt{GPIO00}, a ESP programator (\texttt{esptool.py}) podržava ponovno pokretanje i prenošenje programske podrške na ploču korištenjem tih signala.
