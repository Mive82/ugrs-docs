...
% RTC
\paragraph{RTC DS3231MZ} broji trenutačno vrijeme koje se komunicira s RPi mikroupravljačem u svrhu prikaza istog na ekranu.
% TSS
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - ESP32 mikroupravljač.}}
    \label{fig:esp32}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Gumbovi za programiranje.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Tranzistori za programiranje.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - spojnica za programiranje.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - pretvarač naponskih razina A.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - TSS.}}
    \label{fig:tss}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - TSS oscilator.}}
    \label{fig:tss_osci}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Pretvarač naponskih razina B.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Tranzistor za ponovno pokretanje TSS-a.}}
    \label{fig:tss_reset}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - MCP.}}
    \label{fig:mcp}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Tranzistor za ponovno pokretanje integriranog kruga MCP...}}
    \label{fig:mcp_reset}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Regulator napona stalnih 5 V.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - 3.3 V regulator napona.}}
    \label{fig:3v3_reg}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Djelitelj napona baterije.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - LE diode za detekciju i otklanjanje grešaka.}}
    \label{fig:debug_leds_power}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{images/sch_UART_debug_led.pdf}
    \caption{\textit{Pregled sheme - LE diode za detekciju i otklanjanje grešaka na UART sabirnici.}}
    \label{fig:debug_leds_uart}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{images/sch_van_debug_led.pdf}
    \caption{\textit{Pregled sheme - LE diode za detekciju i otklanjanje grešaka na VAN sabirnici.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Shema spoja LE dioda za detekciju i otklanjanje žohara na UART sabirnici koja povezuje RPi i ESP32.}}
    \label{fig:rpi}
\end{figure}
% RPi header
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Spojnica za RPi mikroupravljač.}}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Shema spoja RTC modula za RPi mikroupravljač.}}
    \label{fig:rpi_rtc}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Spojnica za LCD ekran.}}
    \label{fig:lcd_power}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Pregled sheme - Spojnica za VAN mrežu automobila.}}
    \label{fig:mini_iso_blue}
\end{figure}

Na slikama \ref{fig:pcb_front} i \ref{fig:pcb_back} se nalazi izgled printane tiskane pločice koja predstavlja sklopovski dio projekta. Sve komponente se nalaze s iste strane kako bi se olakšao i ubrzao proces lemljenja. Radi lakšeg snalaženja je svaka značajnija komponenta naznačena na slici. Oznake se manifestiraju bojama kao i pratećim tekstom. Spojnica J3 označava mjesto gdje se spaja LCD ekran, J2 označava mjesto povezivanja projekta s mrežom auta te J7 označava mjesto s kojeg je moguće programirati ESP32 mikroupravljač.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/pcb_front.png}
    \caption{\textit{Izgled prototipa - PCB prednja strana.}}
    \label{fig:pcb_front}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/pcb_back.jpeg}
    \caption{\textit{Izgled prototipa - PCB zadnja strana.}}
    \label{fig:pcb_back}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textit{Blok dijagram programskog rješenja.}}
...
Kao program za razvoj koristi se ESP-IDF programski okvir koji pruža Espressif. 
...
Nakon što postavimo pragove za minimalnu i maksimalnu duljinu signala, periferni uređaj može generirati prekid (eng. \textit{interrupt}) svaki put kada detektira sekvencu \textbf{EOF}.

\noindent
Kada se detektira sekvenca \textbf{EOF}, RMT periferalni upravljački program generira prekid (eng. \textit{interrupt}). 
...
Nakon provjere kontrolnog zbroja (eng. \textit{checksum}), izdvajaju se identifikator i podatci, 
stavljaju se u novi međuspremnik i postavljaju u red glavnog zadatka (eng. \textit{Main task}) za daljnju obradu. 
Logika primanja inspirirana je Peter Pinterovom Arduino bibliotekom \cite{esp32_rmt_van} i prilagođena je arhitekturi programske podrške.
...
Za slanje VAN okvira prema vozilu se koristi TSS463C. 
TSS je zapravo dodatnih 128 bajtova RAM-a kojima se mora upravljati, budući da se mora kontrolirati sva memorija korištena za slanje i primanje podataka.
Trenutačno se TSS koristi samo za slanje paketa CD izmjenjivača kako bi se mogao emulirati, te za slanje zahtjeva za resetiranje stanja putnog računala. 
Poruke CD mjenjača su poruke s neposrednim odgovorom (vidi \ref{subsec:message-types}), dok je zahtjev za resetiranje stanja putnog računala normalni okvir s zahtjevom za potvrdom (ACK). 
Paketi CD mjenjača šalju se periodično, svake sekunde, inače će ugrađeni radio smatrati da CD mjenjača nije prisutan i onemogućiti ga.
...
U ovom slučaju slučaju korišten je isti ADC pin koji se koristi za mjerenje napona baterije kako bi se ESP probudio. 
Zbog naponskog raspona nožice, ne može se koristiti jednostavno buđenje putem GPIO-a, već se mora koristiti ULP koprocesor za mjerenje napona i buđenje ESP-a kada napon prijeđe određeni prag.
...
Prvi način je pisanje asembler koda u zasebnoj datoteci i njegovo integriranje u datoteku firmwarea. 
...
Odabrana je druga opcija jer je znatno jednostavnija.

\begin{table}[H]
  \caption{\textit{Tablica stanja auta.}}
