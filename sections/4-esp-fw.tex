...
upravljanje napajanjem također je važan dio programske podrške, budući da prekomjerna potrošnja u stanju mirovanja brzo iscrpljuje akumulator vozila.
...
Podaci su polje struktura čija definicija je prikazana u kodu \ref{lst:rmt_symbol_word_t}.
...
Pogledajte kod \ref{lst:example-van-frame} za primjer strukture VAN okvira. 
...
Kada glavna petlja (eng. \textit{Main loop}) detektira promjenu u globalnim međuspremnicima podataka (primljen VAN okvir, promjena napona baterije itd.), 
...
\bigskip
\noindent
Događaji su definirani kao struktura (Kod \ref{lst:mive_global_event}) s identifikatorom događaja (Kod \ref{lst:enum-mive_global_event_t}) i pokazivačem na podatke. 
Podaci ovise o primljenom događaju.
...
Odabrali smo drugu opciju jer je znatno jednostavnija. Pogledajte \ref{lst:ulp-deep-sleep} za ULP kod korišten u dubokom snu.
...
if(ret == pdPASS)
{
  ...
  \end{minted}
  \caption{Primjer upravljanja događajima}
\end{listing}